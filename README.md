...
7. **Evaluation on every take** — a take is not selectable or reusable until a facts record (the cut's own declared facts, answered with probabilities and regions) and a judge record clear the project's gate; in autonomous mode `scripts/autopilot.py` runs the whole loop and stops only to have a take looked at or the next recipe written.
