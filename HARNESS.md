...
| **The pasted element** | One object in the frame is drawn from its own eye level — a top surface visible on something well above the horizon — so it reads as rendered separately and composited in | The frame's invented element is the one with no reference to sit in; everything copied from a sheet inherits that sheet's viewpoint and agrees with itself | One capped question: find the horizon from two objects you trust, then check everything else against it, especially whatever the frame had to invent |
| **The summoned agent** | The frame contains a person, a pair of hands or a tool the production never cast, and every declared fact is still present and correct | A verb implies an actor: "the sculpturing goes on" asks for a sculptor, and a rubric built entirely from *is the declared thing there* cannot see anything that was added | One capped question asked the other way round — name everything in the frame, strike off what the cut declares, and answer on what is left. Describe transformations without an agent where the story has none |
| **The detail that is not there to have** | A capped anatomy question rejects three frames in a row because a hand thirty pixels across has no countable fingers | The question was written for a frame where the subject fills it, and is then asked of the same subject at a tenth the size; in a relief-print idiom a small hand *is* a paddle with a thumb | The instruction says to measure first: under about 60 pixels a hand cannot carry five fingers in any idiom, so the answer is `not_visible` with the size stated. "Nothing to resolve" is already a real answer; scale is one of the ways a thing is not there to be seen |
